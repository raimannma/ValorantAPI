...
@responses.activate
@pytest.mark.asyncio
async def test_get_mmr_details_by_name(version: str, region: str, name: str, tag: str):
...
@responses.activate
@pytest.mark.asyncio
async def test_get_mmr_details_by_puuid(version: str, region: str, puuid: UUID):
    ...
    getattr(valo_api, f"get_mmr_details_by_puuid_{version}")(region=region, puuid=puuid)
    assert len(responses.calls) == 1
